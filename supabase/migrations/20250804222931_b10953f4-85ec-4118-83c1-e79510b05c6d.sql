-- Fix transaction type constraint to include bet_winnings
ALTER TABLE public.transactions 
DROP CONSTRAINT transactions_type_check;

ALTER TABLE public.transactions 
ADD CONSTRAINT transactions_type_check 
CHECK (type = ANY (ARRAY['buy_shares'::text, 'sell_shares'::text, 'bet_placed'::text, 'bet_payout'::text, 'bet_winnings'::text, 'deposit'::text, 'withdrawal'::text]));

-- Update the complete_fight_with_stats function to use correct transaction type
...
BEGIN
  ...
  LOOP
    IF bet_record.fighter_id = _winner_id THEN
      ...
      -- Use bet_payout instead of bet_winnings
